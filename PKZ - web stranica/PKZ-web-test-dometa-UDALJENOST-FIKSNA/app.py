    ...
    with get_conn() as conn:
        if koristi_postgres():
            ...
            cur.execute(
                ...
                SET mobile_lat = COALESCE(mobile_lat, %s),
                    mobile_lon = COALESCE(mobile_lon, %s),
                    distance_m = COALESCE(distance_m, %s),
                    gps_accuracy_m = COALESCE(gps_accuracy_m, %s)
                WHERE id = %s
                """,
            ...
            cur = execute(
                ...
                """
                UPDATE range_test_packets
                SET mobile_lat = COALESCE(mobile_lat, ?),
                    mobile_lon = COALESCE(mobile_lon, ?),
                    distance_m = COALESCE(distance_m, ?),
                    gps_accuracy_m = COALESCE(gps_accuracy_m, ?)
                WHERE id = ?
                """,
                (lat, lon, distance_m, accuracy_m, packet_id),
            )
